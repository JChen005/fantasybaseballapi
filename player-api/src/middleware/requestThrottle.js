...
function getThrottleOptions() {
  return {
    windowMs: parsePositiveInteger(
      process.env.REQUEST_THROTTLE_WINDOW_MS,
      DEFAULT_WINDOW_MS,
    ),
    maxRequests: parsePositiveInteger(
      process.env.REQUEST_THROTTLE_MAX,
...
function requestThrottle(req, res, next) {
  ...
  // note to self: requestThrottle mounts first then requireLicense in playerRoutes
  // trust proxy set to true for Vercel
  ...
  res.setHeader(
    "RateLimit-Reset",
    String(Math.ceil((bucket.startedAt + windowMs) / 1000)),
  );

  if (bucket.count > maxRequests) {
    res.setHeader(
      "Retry-After",
      String(Math.ceil((bucket.startedAt + windowMs - now) / 1000)),
    );
